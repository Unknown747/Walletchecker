...
function parseCount(args = process.argv.slice(2)) {
  ...
  const rawCount = countArg ? countArg.split("=")[1] : args.find((a) => /^\d+$/.test(a));
...
function generateSeedWallet(Ed25519Keypair) {
  ...
  const { address: btcAddress } = bitcoin.payments.p2wpkh({ pubkey: Buffer.from(btcChild.publicKey) });
  const { key: solKey } = derivePath(SOL_PATH, seed.toString("hex"));
...
if (require.main === module) {
  generateWallets()
    .then(({ count }) => console.log(`Generated ${count} seed wallets (all chains). Addresses saved to data/.`))
